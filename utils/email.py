...
class EmailUserNotification(object):
    def __init__(self, user, host):
        ...
        host = f'https://{host}'
        self.base_context = {
            'logo_png': f'{host}/{settings.BITHR_LOGO_PNG}',
            'facebook_url': settings.FACEBOOK_URL,
            'facebook_png': f'{host}/{settings.FACEBOOK_PNG}',
            'twitter_url': settings.TWITTER_URL,
            'twitter_png': f'{host}/{settings.TWITTER_PNG}',
            'telegram_url': settings.TELEGRAM_URL,
            'telegram_png': f'{host}/{settings.TELEGRAM_PNG}',
            'youtube_url': settings.YOUTUBE_URL,
            'youtube_png': f'{host}/{settings.YOUTUBE_PNG}',
            'google_play_png': f'{host}/{settings.GOOGLE_PLAY_PNG}',
            'apple_store_png': f'{host}/{settings.APPLE_STORE_PNG}',
        }

    @classmethod
    def send_email(cls, subject, body, html, receiver, sender=settings.EMAIL_NOTIFICATION_SENDER, is_background=True):
        try:
            if is_background:
